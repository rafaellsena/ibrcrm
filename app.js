...
async function boot() {
  const [muns, series] = await Promise.all([
...
function renderMapas(main) {
  ...
  for (const m of municipiosFiltrados()) {
    const row = linhaAno(m.code_muni, state.anoAnalise);
    if (!row || !m.lat || !m.lon) continue;
    const classe = eixo ? row[eixo.col] : classifica01(row[2]);
    const marker = L.circleMarker([m.lat, m.lon], {
      radius: 3, weight: 0, fillColor: cores[classe] ?? '#ccc', fillOpacity: 0.75,
    }).bindTooltip(`<b>${m.nome} (${m.uf})</b><br>${eixo ? eixo.nome : 'Índice Geral'}: ${CLASS_LABELS[classe]}`);
